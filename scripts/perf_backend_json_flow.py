...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backend JSON callback performance test (answer write path)."
    )
    ...
    parser.add_argument("--question-text", default="Backend perf prep question.")
    parser.add_argument("--answer-text", default="Backend perf callback answer.")
...
def main() -> None:
    ...
    for shard in range(1, shard_count + 1):
        conv_status, conv_data = post_json(
            ...
            {"title": f"perf-backend-json-shard-{shard}"},
            token=token,
        )
    ...
    print("Preparing question tasks for callback test...")
    ...
    for seq in range(1, args.requests + 1):
        ...
        ask_status, ask_data = post_json(
            f"{args.base_url}/messages/question",
            {
                ...
                "request_id": f"perf-back-q-{run_id}-{seq}",
            },
            token=token,
        )
        if ask_status != 200 or ask_data.get("code") != 0:
            print("Prepare question failed at seq", seq, ask_status, ask_data)
            return
        prepared.append(
            (
                conversation_id,
                ask_data["data"]["id"],
                ask_data["data"]["generation_task_id"],
            )
        )
    ...
    backend_payload_template = {
        ...
        "backend_task_id": "perf-back-job-<run_id>-<seq>",
        "model_name": "perf-model",
        "answer_text": args.answer_text,
        "answer_request_id": "perf-back-answer-<run_id>-<seq>",
        "assets": [
            {
                "asset_type": "image",
                "asset_url": "https://example.com/perf/image.png",
                "mime_type": "image/png",
                "title": "perf image",
                "sort_no": 1,
            ...
            {
                "asset_type": "mindmap",
                "asset_url": "https://example.com/perf/mindmap.json",
        ...
        "meta_json": {"source": "perf-backend-json-flow"},
    }
    print("Backend callback payload template:")
    print(json.dumps(backend_payload_template, ensure_ascii=False, indent=2))
    ...
    def worker(seq: int, conversation_id: int, task_id: int) -> tuple[bool, float, int, dict]:
        payload = {
            "conversation_id": conversation_id,
            "generation_task_id": task_id,
            "backend_task_id": f"perf-back-job-{run_id}-{seq}",
            "model_name": "perf-model",
            "answer_text": f"{args.answer_text} [{seq}]",
            "answer_request_id": f"perf-back-answer-{run_id}-{seq}",
            "assets": [
                {
                    "asset_type": "image",
                    "asset_url": f"https://example.com/perf/image-{seq}.png",
                    "mime_type": "image/png",
                    "title": "perf image",
                    "sort_no": 1,
                    "meta_json": {"width": 1024, "height": 768},
                ...
                {
                    "asset_type": "mindmap",
                    "asset_url": f"https://example.com/perf/mindmap-{seq}.json",
                    "mime_type": "application/json",
                    "title": "perf mindmap",
                    "sort_no": 2,
                    "meta_json": {"format": "xmind"},
            ...
            "meta_json": {"source": "perf-backend-json-flow", "seq": seq},
        }
        t0 = time.perf_counter()
        status_code, data = post_json(
            f"{args.base_url}/callbacks/model-answer",
            payload,
    ...
    try:
        with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            futures = [
                executor.submit(worker, seq, conversation_id, task_id)
                for seq, (conversation_id, _question_id, task_id) in enumerate(prepared, start=1)
            ]
    ...
    print("\n=== Backend JSON Callback Perf Summary ===")
